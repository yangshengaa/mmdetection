"""
item level partition: partition deepfashion2 data set into subcategories.
- 12 different styles (scale, occlusion, zoom_in, viewpoint)

adapted based on deepfashion2_to_coco.py
"""
...
parser.add_argument("--path", default=None, type=str, help='the path to read and parse')

args = parser.parse_args()

# ============== get filter functions =============
def filter_func(item, category, level):
    """
    filter annotations of a particular category
    :param category: scale, occlusion, zoom_in, viewpoint
    :param level: 1, 2, 3
    """
    return item[category] == level

filter_list = []
for cat in ["scale", "occlusion", "zoom_in", "viewpoint"]:
    for level in [1, 2, 3]:
        filter_list.append(partial(filter_func, category=cat, level=level))
...
dataset_list = [create_empty_dataset() for _ in range(12)]
has_image = [False] * 12
...
ann_path = os.path.join(args.path, 'annos')

img_ids = [name.split(".")[0] for name in os.listdir(img_path)] 

for num_str in img_ids:
    num = int(num_str)
    json_name = os.path.join(ann_path, num_str +'.json')
    image_name = os.path.join(img_path, num_str +'.jpg')
        ...
        with open(json_name, 'r') as f:
            ...
            for i in temp:
                if i == 'source' or i == 'pair_id':
                    ...
                    for k, (dataset, cur_filter_func) in enumerate(zip(dataset_list, filter_list)):
                        # if not this category, skip adding
                        if not cur_filter_func(temp[i]): continue

                        # if it has this category, mark True 
                        has_image[k] = True
            ...
            # loop through each category 
            for dataset, cur_has_image in zip(dataset_list, has_image):
                if cur_has_image:
                    dataset['images'].append({
                        'coco_url': '',
                        'date_captured': '',
                        'file_name': num_str + '.jpg',
                        'flickr_url': '',
                        'id': num,
                        'license': 0,
                        'width': width,
...
for cat in ["scale", "occlusion", "zoom_in", "viewpoint"]:
    for level in [1, 2, 3]:
        ...
        json_name = os.path.join(args.path, f'annotation_coco_{cat}_{level}.json')
